...
@task
def add_features_lr(df_train, df_val):
...
@task
def train_lr(X_train, X_val,y_train, y_val):
...
def main(train_path ="./data/green_tripdata_2021-02.parquet",
        val_path ="./data/green_tripdata_2021-03.parquet"):
    mlflow.set_tracking_uri("sqlite:///mlflow.db")
    mlflow.set_experiment("nyc-taxi-experiment")
    X_train = read_dataframe(train_path)
    X_val = read_dataframe(val_path)
    # LR
    X_train, X_val, y_train, y_val = add_features_lr(X_train, X_val).result()
    train_lr(X_train, X_val, y_train, y_val)
